...
const PerfilUsuario = () => {
    ...
    const loadPerfilsUsuario = () => {
        perfilUsuarioService
            .listarTodos()
            .then((response) => {
                console.log('Fetched PerfilUsuario:', response.data);
                setPerfilsUsuario(response.data);
            })
            .catch((error) => {
                console.error('Error fetching PerfilUsuario:', error);
                toast.current?.show({
                    severity: 'error',
                    summary: 'Erro!',
                    detail: 'Erro ao carregar Perfils Usuário!',
                    life: 3000
                });
            });
    };
    ...
    const savePerfilUsuario = () => {
        ...
        if (perfilUsuario.perfil && perfilUsuario.usuario) {
            if (!perfilUsuario.id) {
                perfilUsuarioService
                    ...
                    .catch((error) => {
                        console.error('Erro ao inserir Perfil Usuário:', error);
                        toast.current?.show({
                            severity: 'error',
                            summary: 'Erro!',
                ...
                perfilUsuarioService
                    .atualizar(perfilUsuario.id, perfilUsuario)
                    .then((response) => {
                        loadPerfilsUsuario();
                        setPerfilUsuarioDialog(false);
                        setPerfilUsuario(perfilUsuarioVazio);
                    ...
                    .catch((error) => {
                        console.error('Erro ao atualizar Perfil Usuário:', error);
                        toast.current?.show({
                            severity: 'error',
                            summary: 'Erro!',
            ...
            toast.current?.show({
                severity: 'warn',
                summary: 'Atenção!',
                detail: 'Perfil e Usuário são obrigatórios!',
                life: 3000
            });
        }
    };
    ...
    const deletePerfilUsuario = () => {
        ...
        if (perfilUsuario.id) {
            perfilUsuarioService
                ...
                .catch((error) => {
                    console.error('Erro ao deletar Perfil Usuário:', error);
                    toast.current?.show({
                        severity: 'error',
                        summary: 'Erro!',
                        detail: `Erro ao deletar! ${error.message || ''}`,
                        life: 5000
                    });
                });
        }
    ...
    const deleteSelectedPerfilsUsuario = () => {
        Promise.all(
            ...
            .catch((error) => {
                console.error('Erro ao deletar Perfils Usuário:', error);
                toast.current?.show({
                    severity: 'error',
                    summary: 'Erro!',
                    detail: `Erro ao deletar! ${error.message || ''}`,
    ...
    const onInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, name: string) => {
        ...
        setPerfilUsuario((prevPerfilUsuario: any) => ({
            ...prevPerfilUsuario,
            [name]: val
        }));
    };
